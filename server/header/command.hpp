...
using std::ostream;
using std::cout;
static QRegularExpression space = QRegularExpression("^\\s+$");
static QVector<QString> operators  = QVector<QString>{ "=",":=","+","-","*","/","%","!","!=","<","<=",">",">=" };
static QVector<QString> separators = QVector<QString>{ ",",";",".","\'","\"","(",")","{","}" };
static QVector<QString> keywords   = QVector<QString>{ "unset","set","write","ip","port","load","terminator","assert"};
struct Word{
    QString type;
    QString body;
    Word* prev;
    Word* next;
//    friend ostream& operator <<(ostream& out, const Word& p)
//    {
//        out<<p.type<<" "<<p.body;
//        return out;
//    }
